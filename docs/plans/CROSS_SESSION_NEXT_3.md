...
- **新 Top 3（取自 ADR-CK-003 §7，依風險/價值排序）**：
  1. **S3 — Meta 跨平臺統整管道**（讀坤哥 Memory Wiki 摘要 → Meta briefing）：純新增、不動 dispatch、對「meta=AaaP 大腦」最有意義。**✅ 設計契約已寫** [`s3-meta-federation-briefing-design.md`](s3-meta-federation-briefing-design.md)（三段：A 源=CK_Missive 開 `/api/ai/memory/digest`＝瓶頸／B 管道=Hermes bridge 薄 action／C 匯=awakening 折 briefing）。
     - **✅ 2026-06-03 段 B 已預寫並 live 探針**（CK_Hermes session）：`memory_digest` action + GET query string 組裝加入 live `query.py`（備份 `.bak.20260603-pre-s3b`）；探針證「機制就緒、後端回 SPA HTML＝等段 A」，`health` 回歸正常，未動 dispatch/未重啟。發現**源頭治理落差**：live `scripts/query.py` vs CK_Missive 部署包 `tools.py` 兩套未同源（詳見 s3 設計 §2 段 B）。
     - **下一動＝段 A 屬 CK_Missive session**（先開 digest 端點）；段 A 完成後 ① 段 B live 驗回真 `digest_text` ② 段 C awakening collector ③ 決定 canonical 源（統一 live↔部署包）。
  ...
  3. **dispatch 可靠度 runtime 決策**（非 SOUL/config）：在 ① tool_choice 強制 ② 換 tool-calling 更穩模型 ③ gateway post-process 三者間選一條做有界 spike。**β spike 已證 α(原生 tool)/MCP 皆非正解，勿再 blind 試 prompt/tool 形式**。
     - **2026-06-03 ① 已實作+live 測試 → ⛔ 否決（[`adr-ck-005-dispatch-tool-choice-forcing.md`](adr-ck-005-dispatch-tool-choice-forcing.md) rejected）**：根因坐實＝`run_agent.py` `tool_choice` 出現 0 次、單一建構點 `_build_api_kwargs:9795` 傳 tools 不傳 tool_choice。實作 patch（呼叫點注入 + helper，env/toggle 雙閘）並部署 live A/B：debug log **鐵證強制正確套用**（每首輪 `FORCED tool_choice=terminal`），但 **groq llama-3.3-70b 不可靠遵守**——7 樣本 4/7 真發 tool_call、3/7 吐成畸形文字（`ilorc` 亂碼 + tool-call-as-text，與 β spike 同源）。**4/7≈57% 落在 baseline 區間，①未提升可靠度→否決**。已還原 pristine（md5 一致）、baseline GO 維持。
     - **⚠️ 2026-06-03 查證 ERRATA（:9119 dashboard，推翻下方①否決）**：ADR-CK-005 受測模型實為 **qwen2.5:7b 非 groq**——`active=meta`，meta profile config 主模型停 5/22（6/2~6/3 改的都是 root config，對 /v1 無效）；今日 9 session（含 spike 7 樣本）`billing` 全 ck-ollama、**0 筆 groq**。∴「①對 groq 無效」**不成立，groq 70b tool_choice 從未測**。~~新首選＝先把 meta profile config 主模型切真 groq 70b 再 A/B 重測~~ → **2026-06-04 已執行並否決**：groq 真生效（ck-ollama 無 70b、實走 groq.com 坐實），但 **groq 免費 tier on_demand TPM 12k < hermes 每請求 20,427 token → 首次後全 413/502，不可行**；已還原 qwen（端到端複驗 1821 筆 GO）。∴ **groq 路需付費 tier 或削 prompt<12k**；現實首選回 **③ gateway post-process**（qwen 上、不受 TPM 影響）。見 [`2026-06-03-hermes-ui-integration-review.md`](2026-06-03-hermes-ui-integration-review.md) §2.5。
     - **方向重訂（以下為 qwen 受測下的歷史結論，待 groq 重測後修正）**：~~③ gateway post-process 升首選~~（model-agnostic）；② 換更穩模型（涉成本）。dispatch 維持現況 terminal ~50-75% GO + 失敗重試。
