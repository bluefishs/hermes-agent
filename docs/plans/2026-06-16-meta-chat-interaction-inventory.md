...
## 5. D-α/D-β SOUL 實測結果（2026-06-16，負向，已還原）

依使用者授權「D-α+D-β 一起改」，於 `profiles/meta/SOUL.md` 加：①回憶優先讀策展 wiki、session_search 僅補充 ②反捏造 recall 硬規。備份 `SOUL.md.bak.20260616-pre-recall`。**live 測（無需重啟即生效）→ 負向**：
- **D-β 反捏造失效**：問「我們上週討論的那個專案叫什麼名字？」（無此事）→ qwen 仍**捏造**「智慧農田管理平臺」（從未討論），無視硬規。
- **D-α 觸發慢檢索**：新規則使 agent 真去 wiki/session_search 檢索 → 回憶類對話 **113–280s+**（一次 280s 超時）。
- ∴ 與已知一致：**prompt 層規則在 qwen2.5:7b 上不可靠**（同 dispatch ~70%、ADR-CK-003 prose 強化無效）。反捏造尤其需模型遵守，prompt 哄不動。
- **處置**：**已還原 SOUL 至備份**（md5 校驗一致），不在生產留下不生效又增延遲的人格改動。

**修正結論**：meta chat 深度記憶的瓶頸**坐實在模型強度（D-δ），非 prompt/管路**。深化的真正槓桿＝(a) 模型升級（groq 付費 tier／更強本地模型，TPM 牆待解）或 (b) **可機械後處理的部分才有效**（如 R1 繁簡能 post-process；但「捏造記憶」無法機械偵測，必須靠模型）。短期：維持現狀（對話內連貫＋R2 寫記憶已 live、夠用），勿再投入 prompt 層 recall 強化。
