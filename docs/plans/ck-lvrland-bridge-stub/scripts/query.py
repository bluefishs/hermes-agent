...
# ── Skill 設定 — ck-lvrland-bridge ──────────────────────
SKILL_NAME = "ck-lvrland-bridge"
DEFAULT_BASE_URL_ENV = "LVRLAND_BASE_URL"
DEFAULT_BASE_URL = "https://lvrland.cksurvey.tw"  # ⚠️ 待 CF Tunnel 上線
TOKEN_ENV = "LVRLAND_API_TOKEN"  # 可選（dev mode 不需）
TIMEOUT_S = 60
...
# ── Action 對應 endpoint — ck-lvrland-bridge ────────────
ACTION_HANDLERS: dict[str, dict[str, Any]] = {
    ...
    "ai_query": {
        # NL 查詢估價/實價/行政區（Groq+Ollama RAG）
        # 命中地圖關鍵字 + 行政區時回 {type: "tool_call", tool_name: "map_highlight"}
        "method": "POST",
        "path": "/api/v1/ai/query",
        "body_required": True,
        "auth_required": False,  # LvrLand 預設 dev mode（token 可選）
        "expected_args": ["question"],
    },
    "price_trends": {
        # 行政區房價/成交量時序資料
        # body: {"districts": ["中壢區", "桃園區"]} 或 CLI: --districts "中壢區,桃園區"
        "method": "POST",
        "path": "/api/v1/analytics/price-volume-trends",
        "body_required": True,
        "auth_required": False,
        "expected_args": ["districts"],
    },
}
