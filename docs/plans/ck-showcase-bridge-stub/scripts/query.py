...
# ── Skill 設定 — ck-showcase-bridge ─────────────────────
SKILL_NAME = "ck-showcase-bridge"
DEFAULT_BASE_URL_ENV = "SHOWCASE_BASE_URL"
DEFAULT_BASE_URL = "https://showcase.cksurvey.tw"  # ⚠️ 待 CF Tunnel 上線
TOKEN_ENV = "SHOWCASE_API_TOKEN"  # 大部分為 public read，少數 write 需 token
TIMEOUT_S = 30
...
# ── Action 對應 endpoint — ck-showcase-bridge（8 actions）─
ACTION_HANDLERS: dict[str, dict[str, Any]] = {
    ...
    "managed_projects": {
        "method": "POST",
        "path": "/api/overview/projects",
        "body_required": True,  # body 可為空 {}
        "auth_required": False,
        "expected_args": [],
    },
    "adr_map_query": {
        # ADR 跨 repo 地圖查詢
        "method": "POST",
        "path": "/api/adr-map/query",
        "body_required": True,
        "auth_required": False,
        "expected_args": [],
    },
    "governance_health": {
        # 治理健康度（per project_id）
        "method": "POST",
        "path": "/api/overview/governance-health",
        "body_required": True,
        "auth_required": False,
        "expected_args": [],
    },
    "platform_metrics": {
        # 平臺指標（window=30d default）
        "method": "POST",
        "path": "/api/overview/platform-metrics",
        "body_required": True,
        "auth_required": False,
        "expected_args": [],  # window 是 optional
    },
    "skills_sync_status": {
        # Skill 同步狀態（per project_id）
        "method": "POST",
        "path": "/api/skills/sync-status",
        "body_required": True,
        "auth_required": False,
        "expected_args": ["project_id"],
    },
    "agents_list": {
        # Agent 清單（per project_id）
        "method": "POST",
    ...
    "security_scan": {
        # 安全掃描（destructive — safe_mode 預設 dry-run）
        "method": "POST",
        "path": "/api/security/scan",
        "body_required": True,
        "auth_required": True,  # 寫入需 token
        "expected_args": [],
    },
    "sso_status": {
        # SSO 狀態（per project_id）
        "method": "POST",
        "path": "/api/system-config/sso-status",
        "body_required": True,
        "auth_required": False,
        "expected_args": ["project_id"],
    },
