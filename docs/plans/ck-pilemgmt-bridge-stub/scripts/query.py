...
# ── Skill 設定 — ck-pilemgmt-bridge ─────────────────────
SKILL_NAME = "ck-pilemgmt-bridge"
DEFAULT_BASE_URL_ENV = "PILE_BASE_URL"
DEFAULT_BASE_URL = "https://pile.cksurvey.tw"  # ⚠️ 待 CF Tunnel 上線
TOKEN_ENV = "PILE_API_TOKEN"
TIMEOUT_S = 60
...
# ── Action 對應 endpoint — ck-pilemgmt-bridge ───────────
ACTION_HANDLERS: dict[str, dict[str, Any]] = {
    ...
    "ai_query": {
        # ⚠️ PileMgmt 後端尚未實作 /api/ai/query（per ADR-0023）
        # 呼叫此 action 會回 backend_endpoint_missing
        "method": "POST",
        "path": "/api/ai/query",
        "body_required": True,
        "auth_required": True,
    ...
    "celery_status": {
        # 工作佇列彙總（active/scheduled/reserved/workers）
        # 可選 --queue_filter "queue_name"
        "method": "POST",
        "path": "/api/celery/status",
        "body_required": True,
        "auth_required": True,
        "expected_args": [],  # queue_filter 是 optional
    },
}
