#!/usr/bin/env python3
"""Manual verification helper for the 4 ADR-0020 Phase 1 native tool bridges.
...
pilemgmt}_tool.py``. Designed for use after a CK_AaaP / receiving-repo
session has injected the env vars into ``hermes-stack/.env`` or the operator
shell, so you can confirm at a glance which bridges are reachable.

Usage::

    python scripts/verify-bridges.py [--bridge missive|observability|showcase|pilemgmt|all]
...
- OBSERVABILITY_GRAFANA_URL             (+ OBSERVABILITY_GRAFANA_USER/PASS optional)
...
def probe_observability_prom() -> ProbeResult:
    ...
    base = os.environ.get("OBSERVABILITY_PROMETHEUS_URL")
    if not base:
        return ProbeResult(
            "observability", "prometheus_query", "SKIP", "OBSERVABILITY_PROMETHEUS_URL not set"
        )
    base = base.rstrip("/")
...
def probe_observability_loki() -> ProbeResult:
    return _probe(
        "observability", "loki_labels", "OBSERVABILITY_LOKI_URL", "/loki/api/v1/labels"
    )


def probe_observability_grafana() -> ProbeResult:
    headers = {}
    user = os.environ.get("OBSERVABILITY_GRAFANA_USER")
    password = os.environ.get("OBSERVABILITY_GRAFANA_PASS")
    if user and password:
        cred = base64.b64encode(f"{user}:{password}".encode()).decode()
        headers["Authorization"] = f"Basic {cred}"
    return _probe(
        "observability", "grafana_health", "OBSERVABILITY_GRAFANA_URL", "/api/health", headers
    )


def probe_observability_alert() -> ProbeResult:
    return _probe(
        "observability", "alerts_active", "OBSERVABILITY_ALERTMANAGER_URL", "/api/v2/alerts"
    )
...
def run(bridge_filter: str) -> int:
    bridges = ["missive", "observability", "showcase", "pilemgmt"]
...
def main(argv: list[str]) -> int:
    ...
    p.add_argument(
        "--bridge",
        default="all",
        choices=["missive", "observability", "showcase", "pilemgmt", "all"],
