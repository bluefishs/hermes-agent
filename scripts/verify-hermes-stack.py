...
Three layers of probe, in increasing depth:
...
def probe_l3_dispatch(host: str, port: int, api_key: Optional[str], prompt: str) -> ProbeResult:
...
def main() -> int:
    ...
    if os.environ.get("HERMES_E2E_REAL_STACK") == "1":
        if results[0].status == "OK":
            ...
            results.append(probe_l3_dispatch(args.host, args.gateway_port, api_key, prompt))
        else:
